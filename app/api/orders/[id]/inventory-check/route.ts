import { NextResponse } from "next/server"
import { supabase } from "@/lib/supabase"

export async function GET(request: Request, { params }: { params: { id: string } }) {
  try {
    const orderId = Number.parseInt(params.id)

    if (!orderId) {
      return NextResponse.json({ success: false, error: "Invalid order ID" }, { status: 400 })
    }

    // Fetch order items
    const { data: orderItems, error: orderItemsError } = await supabase
      .from("order_items")
      .select("menu_item_id, quantity")
      .eq("order_id", orderId)

    if (orderItemsError) {
      // Check for rate limit error
      if (orderItemsError.message && orderItemsError.message.includes("Too Many Requests")) {
        console.warn("Rate limit hit when fetching order items")
        return NextResponse.json({ success: false, error: "Rate limit exceeded" }, { status: 429 })
      }

      console.error("Error fetching order items:", orderItemsError)
      return NextResponse.json({ success: false, error: orderItemsError.message }, { status: 500 })
    }

    if (!orderItems || orderItems.length === 0) {
      return NextResponse.json({ isOk: true, missingIngredients: [] })
    }

    // Get all menu item IDs
    const menuItemIds = orderItems.map((item) => item.menu_item_id)

    // Fetch recipes for these menu items
    const { data: recipes, error: recipesError } = await supabase
      .from("recipes")
      .select("id, menu_item_id")
      .in("menu_item_id", menuItemIds)

    if (recipesError) {
      // Check for rate limit error
      if (recipesError.message && recipesError.message.includes("Too Many Requests")) {
        console.warn("Rate limit hit when fetching recipes")
        return NextResponse.json({ success: false, error: "Rate limit exceeded" }, { status: 429 })
      }

      console.error("Error fetching recipes:", recipesError)
      return NextResponse.json({ success: false, error: recipesError.message }, { status: 500 })
    }

    if (!recipes || recipes.length === 0) {
      return NextResponse.json({ isOk: true, missingIngredients: [] })
    }

    // Create a map of menu item ID to recipe ID
    const menuItemToRecipe = recipes.reduce(
      (map, recipe) => {
        map[recipe.menu_item_id] = recipe.id
        return map
      },
      {} as Record<number, number>,
    )

    // Get all recipe IDs
    const recipeIds = recipes.map((recipe) => recipe.id)

    // Fetch recipe ingredients
    const { data: recipeIngredients, error: ingredientsError } = await supabase
      .from("recipe_ingredients")
      .select("recipe_id, ingredient_id, quantity_for_recipe")
      .in("recipe_id", recipeIds)

    if (ingredientsError) {
      // Check for rate limit error
      if (ingredientsError.message && ingredientsError.message.includes("Too Many Requests")) {
        console.warn("Rate limit hit when fetching recipe ingredients")
        return NextResponse.json({ success: false, error: "Rate limit exceeded" }, { status: 429 })
      }

      console.error("Error fetching recipe ingredients:", ingredientsError)
      return NextResponse.json({ success: false, error: ingredientsError.message }, { status: 500 })
    }

    if (!recipeIngredients || recipeIngredients.length === 0) {
      return NextResponse.json({ isOk: true, missingIngredients: [] })
    }

    // Get all ingredient IDs
    const ingredientIds = [...new Set(recipeIngredients.map((ri) => ri.ingredient_id))]

    // Fetch current inventory
    const { data: inventory, error: inventoryError } = await supabase
      .from("ingredients")
      .select("id, name, quantity, unit")
      .in("id", ingredientIds)

    if (inventoryError) {
      // Check for rate limit error
      if (inventoryError.message && inventoryError.message.includes("Too Many Requests")) {
        console.warn("Rate limit hit when fetching inventory")
        return NextResponse.json({ success: false, error: "Rate limit exceeded" }, { status: 429 })
      }

      console.error("Error fetching inventory:", inventoryError)
      return NextResponse.json({ success: false, error: inventoryError.message }, { status: 500 })
    }

    if (!inventory) {
      return NextResponse.json({ isOk: false, missingIngredients: [] })
    }

    // Create a map of ingredient ID to inventory
    const inventoryMap = inventory.reduce(
      (map, item) => {
        map[item.id] = { quantity: item.quantity, name: item.name, unit: item.unit || "" }
        return map
      },
      {} as Record<number, { quantity: number; name: string; unit: string }>,
    )

    // Calculate required ingredients
    const requiredIngredients: Record<number, number> = {}

    for (const orderItem of orderItems) {
      const recipeId = menuItemToRecipe[orderItem.menu_item_id]
      if (!recipeId) continue

      const ingredients = recipeIngredients.filter((ri) => ri.recipe_id === recipeId)
      for (const ingredient of ingredients) {
        const requiredQuantity = ingredient.quantity_for_recipe * orderItem.quantity
        requiredIngredients[ingredient.ingredient_id] =
          (requiredIngredients[ingredient.ingredient_id] || 0) + requiredQuantity
      }
    }

    // Check if we have enough inventory
    const missingIngredients = []
    let isOk = true

    for (const [ingredientId, requiredQuantity] of Object.entries(requiredIngredients)) {
      const id = Number.parseInt(ingredientId)
      const inventoryItem = inventoryMap[id]

      if (!inventoryItem || inventoryItem.quantity < requiredQuantity) {
        isOk = false
        missingIngredients.push({
          name: inventoryItem?.name || `Ingredient #${id}`,
          available: inventoryItem?.quantity || 0,
          required: requiredQuantity,
          unit: inventoryItem?.unit || "",
        })
      }
    }

    return NextResponse.json({ isOk, missingIngredients })
  } catch (error) {
    console.error("Error checking inventory:", error)

    // Check if this is a rate limit error
    if (error.message && error.message.includes("Too Many Requests")) {
      return NextResponse.json({ success: false, error: "Rate limit exceeded" }, { status: 429 })
    }

    return NextResponse.json({ success: false, error: "Internal server error" }, { status: 500 })
  }
}
